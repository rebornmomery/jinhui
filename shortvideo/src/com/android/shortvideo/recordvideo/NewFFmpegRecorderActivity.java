 ...
 */
@SuppressLint("NewApi")
public class NewFFmpegRecorderActivity extends Activity implements OnClickListener, OnTouchListener {
...
//		stateImageView = (ImageView) findViewById(R.id.recorder_surface_state);
	...
	private void initCameraLayout() {
		...
		cameraView = new CameraView(NewFFmpegRecorderActivity.this, cameraDevice);
...
//		topLayout.addView(view,layoutParam2);
		...
		mRecordImage.setOnTouchListener(NewFFmpegRecorderActivity.this);
		
		switchCameraIcon.setOnClickListener(NewFFmpegRecorderActivity.this);
...
////				topLayout.addView(view,layoutParam2);
//				
//				mRecordImage.setOnTouchListener(FFmpegRecorderActivity.this);
//				
...
//		}.execute("start");
	...
	public class AsyncStopRecording extends AsyncTask<Void,Integer,Void>{
		...
		protected void onPreExecute() {
			...
			//创建处理进度条
			creatingProgress= new Dialog(NewFFmpegRecorderActivity.this,R.style.Dialog_loading_noDim);
		...
		private void getFirstCapture(byte[] data){
			...
			captureBitmapPath = Util.createImagePath(NewFFmpegRecorderActivity.this);
	...
	private void showCancellDialog(){
		Util.showDialog(NewFFmpegRecorderActivity.this, "提示", "确定要放弃本视频吗？", 2, new Handler(){
			@Override
			public void dispatchMessage(Message msg) {
				if(msg.what == 1)
					videoTheEnd(false);
	...
	private void handleSurfaceChanged()
	{
		...
		if(Build.VERSION.SDK_INT >  Build.VERSION_CODES.FROYO)
		{
			mCamera.setDisplayOrientation(Util.determineDisplayOrientation(NewFFmpegRecorderActivity.this, defaultCameraId));
	...
	public void onClick(View v) {
		...
		if(v.getId() == R.id.recorder_next){
		...
		}else if(v.getId() == R.id.record_video_title_toggle_flash){
			...
			mCamera.setParameters(cameraParameters);
		}else if(v.getId() == R.id.record_video_title_switch_camera){
			//转换摄像头
		...
		}else if(v.getId() == R.id.record_video_title_back){
			if (recording) 
				showCancellDialog();
			else
				videoTheEnd(false);
