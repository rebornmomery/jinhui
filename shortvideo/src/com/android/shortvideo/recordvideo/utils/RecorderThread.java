...
public class RecorderThread extends Thread{
    ...
    private NewFFmpegRecorderActivity.AsyncStopRecording mAsyncTask;
    ...
    public void run() {
        try {
            if(mByteBuffer == null){
                mByteBuffer = ByteBuffer.allocate(mSize * mTotalFrame);//ByteBuffer.allocateDirect(mSize * mTotalFrame);
    ...
    public void stopRecord(AsyncTask asyncTask){
        mAsyncTask = (NewFFmpegRecorderActivity.AsyncStopRecording)asyncTask;
        mIsStop.set(true);
        try {
            this.join();
        } catch (Exception e) {
